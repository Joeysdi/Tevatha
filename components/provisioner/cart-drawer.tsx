          ...
          >
            ...
            {items.length > 0 && (
              <div className="flex-shrink-0 px-5 py-4 border-t border-border-protocol space-y-3">
                ...
                {/* Card checkout */}
                {cardItems.length > 0 && (
                  <CardCheckoutButton items={cardItems} />
                )}

                {/* USDC checkout — per item */}
                {usdcItems.map((item) => (
...
function CardCheckoutButton({ items }: { items: CartItem[] }) {
  ...
  const handleCheckout = async () => {
    ...
    if (lineItems.length === 0) {
      setError("No Stripe price IDs configured for these items.");
      return;
    }
  ...
  return (
    <div className="space-y-1.5">
      ...
      {error && (
        <p className="font-mono text-[9px] text-red-bright text-center">{error}</p>
      )}
    </div>
  );
}
